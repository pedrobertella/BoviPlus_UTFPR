...
public class PagarSaudeController implements Initializable {
    ...
    public void lista() throws IOException {
        ...
        Main.window = false;
        Parent fxmlDB = FXMLLoader.load(getClass().getResource("TelaInicio.fxml"));
        Scene scene = new Scene(fxmlDB);
        listar.getIcons().add(new Image("/Imagens/Iconew.png"));
        listar.setScene(scene);
        listar.setTitle("ARRUMAR ESTA TELA");
        listar.show();
        Main.window = true;
    }
    ...
    public void carrega() {
        ...
        try {
            ...
            cv = dv.consultaCusto_vacina(cp.getId_contas_pagar());
            vacina_tx.setText(Integer.toString(cv.getId_vacina()));
            busca();
            vuni_tx.setText(Double.toString(cv.getValor()));
            qtd_tx.setText(Integer.toString(cv.getQtd()));
            ...
            ((Stage) ko.getDialogPane().getScene().getWindow()).getIcons().add(new Image(new File("Iconew.png").toURI().toString()));
            ko.setTitle("Atenção!");
            ko.setHeaderText("A conta a pagar fornecida não é de Vacinação, para carregá-la"
    ...
    public void salva() {
        ...
        if (qtd_tx.getText().trim().isEmpty()) {
            ...
            ((Stage) ko.getDialogPane().getScene().getWindow()).getIcons().add(new Image(new File("Iconew.png").toURI().toString()));
            ko.setTitle("Atenção!");
            ko.setHeaderText("Você precisa carregar uma vacinação para salvar...");
            ko.showAndWait().ifPresent(rr -> {
                if (rr == ButtonType.OK) {
                }
            });
            vacina_tx.requestFocus();
            ...
            try {
                ...
                dv = new DAOCusto_vacina();
                if (carregado) {
                    cv.setId_vacina(Integer.parseInt(vacina_tx.getText()));
                    ...
                    cv.setValor(Double.parseDouble(vuni_tx.getText()));
                    cv.setQtd(Integer.parseInt(qtd_tx.getText()));
                    multiplica();
                    cp.setValor(total);
                    cp.setOrigem(3);

                    dp.atualizarContas_pagar(cp.getId_contas_pagar(), cp.getValor(), cp.getData_vencimento(), cp.getData_pagamento());
                    dv.atualizarCusto_vacina(cv.getId_vacina(), cv.getId_contas_pagar(), cv.getQtd(), cv.getValor());
                    ...
                    cv.setId_contas_pagar(dp.proximoID());
                    multiplica();
                    cp.setValor(total);

                    dp.cadastrarContas_pagar(cp);
                    dv.cadastrarCusto_vacina(cv);
    ...
    @FXML
    public void busca() {
        if (vacina_tx.getText().trim().isEmpty()) {
            Main.tocaConfirma();
            Alert ko = new Alert(Alert.AlertType.WARNING);
            ImageView imageView = new ImageView(new Image(new File("warning.png").toURI().toString()));
            ko.setGraphic(imageView);
            ((Stage) ko.getDialogPane().getScene().getWindow()).getIcons().add(new Image(new File("Iconew.png").toURI().toString()));
            ko.setTitle("Atenção!");
            ko.setHeaderText("Para buscar, informe o código da vacinação...");
            ko.showAndWait().ifPresent(rr -> {
                if (rr == ButtonType.OK) {
                }
            });
            ...
            try {
                DAOVacinacao a = new DAOVacinacao();
                Vacinacao v = a.consultaVac(Integer.parseInt(vacina_tx.getText()));
                DAOAnimal da = new DAOAnimal();
                Animal aa = da.consultaAnimal(v.getId_animal());
                nome_tx.setText(aa.getNome());
                loaded = true;
            } catch (Exception e) {
                Main.tocaErro();
                Alert ko = new Alert(Alert.AlertType.ERROR);
                ...
                ((Stage) ko.getDialogPane().getScene().getWindow()).getIcons().add(new Image(new File("Iconew.png").toURI().toString()));
                ko.setTitle("Atenção!");
                ko.setHeaderText("Vacinação não encontrada!");
                ko.setContentText(e.getMessage());
                ko.showAndWait().ifPresent(rr -> {
                    if (rr == ButtonType.OK) {
                    }
